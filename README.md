...
Осталось приблизительным:
- **Шеврон «вправо»** нарисован на CSS — в core-наборе иконок стрелки нет (в Figma `icons/chevron-right` есть,
  но экспорт ассетов был заблокирован настройкой Dev Mode → Allowed directories).
- **Маскот «закладыш»** (кот Bookmate) внутри плейсхолдера и **логотипы магазинов** (`snippet company`:
  ozon / market / alpina / читай-город) — растровые/SVG-ассеты не выгружены; плейсхолдеры показаны как
  цветные плашки. Добавить тем же методом, что и иконки, после включения Allowed directories.
